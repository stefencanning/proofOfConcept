...
Island::Island(float x,float y,float w,float h,float r) {
	position.x = x;
	position.y = y;
	width = h;
	height = w;
...
void Island::Draw(SDL_Renderer* gRenderer, SDL_RendererFlip flipType) {
	...
	SDL_RenderCopyEx(gRenderer,TextureManager::getManager()->islandTexture,NULL,new SDL_Rect(posRec),rotation,NULL,SDL_FLIP_NONE);
}
...
void Island::Update(float timeElapsed){
	...
	void Island::Reset(SDL_Renderer* gRenderer){
		alive = true;
	}